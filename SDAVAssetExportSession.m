...
@property (nonatomic, strong) AVAssetReader *extraReader;
...
@property (nonatomic, strong) AVAssetReaderAudioMixOutput *extraAudioOutput;
@property (nonatomic, strong) AVAssetWriter *writer;
@property (nonatomic, strong) AVAssetWriterInput *videoInput;
@property (nonatomic, strong) AVAssetWriterInputPixelBufferAdaptor *videoPixelBufferAdaptor;
@property (nonatomic, strong) AVAssetWriterInput *audioInput;
@property (nonatomic, strong) AVAssetWriterInput *extraAudioInput;
...
- (void)exportAsynchronouslyWithCompletionHandler:(void (^)(void))handler
{
    ...
    //    [audioTracks addObjectsFromArray:[audioAsset tracksWithMediaType:AVMediaTypeAudio]?:@[]];
...
//    NSString *filePath = [[NSBundle mainBundle] pathForResource:@"audioTest" ofType:@"mp3"];
//    AVURLAsset *audioAsset = [AVURLAsset assetWithURL:[NSURL fileURLWithPath:filePath]];
//    NSArray *newAudioTracks = [audioAsset tracksWithMediaType:AVMediaTypeAudio];
//    NSError *extraReaderError;
//    self.extraReader = [AVAssetReader.alloc initWithAsset:audioAsset error:&extraReaderError];
//    
//    
//    if (newAudioTracks.count > 0 && !extraReaderError) {
//        NSTimeInterval newDuration = MIN(duration,CMTimeGetSeconds(audioAsset.duration));
//        self.extraReader.timeRange = CMTimeRangeMake(kCMTimeZero, CMTimeMake(newDuration, 1));
//        self.extraAudioOutput = [AVAssetReaderAudioMixOutput assetReaderAudioMixOutputWithAudioTracks:newAudioTracks audioSettings:nil];
//        self.extraAudioOutput.alwaysCopiesSampleData = NO;
//        self.extraAudioOutput.audioMix = self.audioMix;
//        if ([self.extraReader canAddOutput:self.extraAudioOutput])
//        {
//            [self.extraReader addOutput:self.extraAudioOutput];
//        } else {
//            // Just in case this gets reused
//            self.extraAudioOutput = nil;
//        }
//    }
    ...
    if (self.extraAudioOutput) {
        self.extraAudioInput = [AVAssetWriterInput assetWriterInputWithMediaType:AVMediaTypeAudio outputSettings:self.audioSettings];
        self.extraAudioInput.expectsMediaDataInRealTime = NO;
        if ([self.writer canAddInput:self.extraAudioInput])
        {
            [self.writer addInput:self.extraAudioInput];
        }
    }
    ...
    if (videoTracks.count > 0) {
        [self.videoInput requestMediaDataWhenReadyOnQueue:self.inputQueue usingBlock:^
        {
            if (![wself encodeReadySamplesFromOutput:wself.videoOutput toInput:wself.videoInput])
            {
                ...
                {
                    videoCompleted = YES;
                    if (audioCompleted && extraAudioCompleted)
                    {
                        [wself finish];
                    }
                }
        ...
        [self.audioInput requestMediaDataWhenReadyOnQueue:self.inputQueue usingBlock:^
         {
             if (![wself encodeReadySamplesFromOutput:wself.audioOutput toInput:wself.audioInput])
             {
                 ...
                 {
                     audioCompleted = YES;
                     if (videoCompleted && extraAudioCompleted)
                     {
                         [wself finish];
                     }
                 }
    ...
    if (!self.extraAudioOutput) {
        extraAudioCompleted = YES;
    } else {
        [self.extraAudioInput requestMediaDataWhenReadyOnQueue:self.inputQueue usingBlock:^
         {
             if (![wself encodeReadySamplesFromOutput:wself.extraAudioOutput toInput:wself.extraAudioInput])
             {
                 @synchronized(wself)
                 {
                     extraAudioCompleted = YES;
...
//	CGSize targetSize = CGSizeMake([self.videoSettings[AVVideoWidthKey] floatValue], [self.videoSettings[AVVideoHeightKey] floatValue]);
    ...
    CGSize updatedSize = __CGSizeApplyAffineTransform(naturalSize, transform);
	videoComposition.renderSize = CGSizeMake(fabs(updatedSize.width), fabs(updatedSize.height));
